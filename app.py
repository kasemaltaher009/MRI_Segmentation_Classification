...
add_bg_and_card("MRI Room in Modern Medical Facility.png")
...
def load_models():
    ...
    segmenter = load_model("BrainSeg.keras")
...
# ----------------------------
# Overlay mask (scale mask up to MRI size)
# ----------------------------
def overlay_mask(image_pil, mask, threshold=0.6, alpha=0.4):
    ...
    mask_resized = mask_img.resize(image_pil.size, Image.NEAREST)

    # Build RGBA overlay
    mask_arr = np.array(mask_resized)
    overlay = np.zeros((mask_resized.size[1], mask_resized.size[0], 4), dtype=np.uint8)
    overlay[:, :, 0] = mask_arr
    overlay[:, :, 3] = (mask_arr > 0) * int(255 * alpha)

    combined = Image.alpha_composite(image_pil.convert("RGBA"), Image.fromarray(overlay))
...
def generate_report(tumor_type, confidence):
    ...
    Just return the short report and nothing else.
